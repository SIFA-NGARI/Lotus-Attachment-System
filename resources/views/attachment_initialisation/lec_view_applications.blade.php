    ...
    <a href="{{route('dashboard')}}">
        ...
        <table id="example" class="table table-striped table-bordered" cellspacing="0" width="100%">
            ...
            <tbody>
                ...
                <?php
                $id = $data->student_id;
                $data2 = DB::table('applications')->where('applications.student_id', '=', $id)->get(); ?>
                @foreach ($data2 as $data2)
                <?php
                $lat = $data2->address_latitude;
                $long = $data2->address_longitude;
                $org_name = $data2->org_name;
                $date = $data2->date;
                $application_id = $data2->id; ?>
                @endforeach
                <?php
                $data3 = DB::table('users')->where('id', '=', $id)->get(); ?>
                @foreach ($data3 as $data3)
                <?php
                $student_id = $data3->id;
                $student_name = $data3->name; ?>
                @endforeach
                ...
                <tr onclick="showMap(<?php echo $lat ?>,<?php echo $long ?> )">
                    ...
                    <td>{{$long}}</td>
                    <td>{{$date}}</td>
                    <td><a id="this1" href="{{ route('view_attachment_details_lec',$application_id)}}">View Details</a></td>
                </tr>
                @endforeach
            </tbody>
